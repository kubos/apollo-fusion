...
impl AppHandler for MyApp {
    ...
    fn on_boot(&self, _args: Vec<String>) -> Result<(), Error> {
        ...
        if let Err(error) = query(&mcu_service, OEM6_POWER, Some(QUERY_TIMEOUT)) {
            error!("Failed to turn on OEM6: {:?}", error);
        };
        if let Err(error) = query(&mcu_service, OEM6_COMM, Some(QUERY_TIMEOUT)) {
            error!("Failed to set OEM6 UART: {:?}", error);
        }
        if let Err(error) = query(&mcu_service, OEM6_PASS, Some(QUERY_TIMEOUT)) {
            error!("Failed to enable OEM6 communication: {:?}", error);            
        };
